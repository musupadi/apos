...
class _loginsState extends State<logins> {
  ...
  LoginData() async {
    ...
    try{
      final url = Uri.parse(getServerName()+Login());
      final request = http.MultipartRequest('POST', url);
      request.headers.addAll(headerBuilderWT());
      request.fields['username'] = controllerUsername.text;
      request.fields['password'] = controllerPassword.text;
      final response = await request.send();
      final respStr = await response.stream.bytesToString();
      print("Testing Zyarga : "+jsonDecode(respStr)['data'][0].toString());
      if(jsonDecode(respStr)['code'] == 0){
        ...
        await prefs.setString('role_name', jsonDecode(respStr)['data'][0]['role_name'].toString());
        await prefs.setString('name', jsonDecode(respStr)['data'][0]['name'].toString());
        await prefs.setString('shop_name', jsonDecode(respStr)['data'][0]['shop_name'].toString());
        ...
        Message('Login Gagal',"Username atau Password anda Salah", context);
      }
    }on TimeoutException catch(e){
      setState(() {
        isLoading = false;
      });
      Message("Login Gagal","Mohon Dicoba Lagi", context);
    } on SocketException catch (e){
      setState(() {
        isLoading = false;
      });
      Message("Login Gagal","Socket Salah : "+e.toString(), context);
    } on Error catch (e){
      setState(() {
        isLoading = false;
      });
      Message("Login Gagal","Error : "+e.toString(), context);
    }
  }
